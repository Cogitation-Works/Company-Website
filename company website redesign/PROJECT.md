...
| 2026-09-24 | Measured on `/products` with the blob live: **p50 6.9ms, p95 7.2ms, worst 7.8ms, zero frames over 32ms.** Raymarching is fill-rate bound, so DPR is capped at 1.35 and the march at 28 steps; desktop-gated and lens-copy-guarded like every other 3D object |
| | *next: client generates §4.6 (3 films + 5 stills) and §4.1 (4 industry stills + manufacturing at 2560px); records the product and client screens, and the 6 client review videos. **Still needed from the client: the Robotics scope, the Sunday-delivery service area and cut-off, and real testimonials** — which now block every product and project page* |
